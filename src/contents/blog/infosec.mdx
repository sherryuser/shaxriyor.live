...
Explanation: Nmap is used for network discovery and security auditing. It can scan for open ports, detect service versions, and even identify the operating system of a target.
Example: Scan for open ports on a target IP.
...
Explanation: Nikto is a web server scanner which checks for dangerous files/CGIs, outdated server software, and other problems.
...
Explanation: Ping is a network utility used to test the reachability of a host on an IP network and measure the round-trip time for messages sent from the originating host.
Example: Ping a host to check if it's online.
...
Explanation: Hping3 is a network tool able to send custom TCP/IP packets and to display target replies like ping does with ICMP replies.
Example: Send SYN packets to port 80 of a target IP.
...
Explanation: Netcat is a versatile networking tool used for reading from and writing to network connections using TCP or UDP.
Example: Create a simple TCP connection to port 80 of a website.
...
Explanation: DMitry is capable of gathering data about a target, such as subdomains, email addresses, uptime info, and more.
Example: Gather email addresses for a domain.
...
Explanation: Tcpdump is a powerful command-line packet analyzer for Unix-like operating systems; it's used for network debugging and traffic monitoring.
Example: Capture packets on the eth0 interface.
...
Explanation: arp-scan sends ARP packets to hosts on the local network and displays any responses, useful for discovering physical devices on a network.
Example: Scan your local network for devices.
...
Explanation: Netdiscover is an active/passive ARP reconnaissance tool, used for scanning networks to discover hosts.
Example: Passively scan for network hosts.
...
Explanation: Wapiti allows you to audit the security of your web applications. It performs "black-box" scans, i.e., it does not study the source code but scans the webpages of the deployed web app.
Example: Scan a website for vulnerabilities.
...
Explanation: theHarvester is used to gather open-source intelligence (OSINT), particularly around domain emails, subdomains, hosts, employee names, and more.
Example: Gather emails related to a domain.
...
Explanation: dig is a tool for querying DNS name servers for information about host addresses, mail exchanges, name servers, and related information.
Example: Query A records for a domain.
...
Explanation: whois is a protocol and utility used to query databases that store the registered users or assignees of an Internet resource, such as a domain name.
Example: Perform a whois lookup for a domain.
